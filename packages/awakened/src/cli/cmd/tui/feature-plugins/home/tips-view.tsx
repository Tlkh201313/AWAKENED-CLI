...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    sessionUsage: configShortcut(props.api, "session.usage"),
    statusView: useCommandShortcut("awakened.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at awakened.ai",
  ...
  "Run {highlight}/learn{/highlight} to extract session learnings into scoped AGENTS.md files",
  "Use {highlight}/voice{/highlight} for dictation — floating bar appears; Enter or click to finish",
  "Say {highlight}obsidian vault{/highlight} to get the easy mcp-obsidian MCP setup (Local REST API plugin + one API key)",
  ...
  "Create {highlight}awakened.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/awakened/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.awakened/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.awakened/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.awakened/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.awakened/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Awakened from reading sensitive files",
  "Use {highlight}awakened run{/highlight} for non-interactive scripting",
  "Use {highlight}awakened --continue{/highlight} to resume the last session",
  "Use {highlight}awakened run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}awakened serve{/highlight} for headless API access to Awakened",
  "Use {highlight}awakened run --attach{/highlight} to connect to a running server",
  "Run {highlight}awakened upgrade{/highlight} to update to the latest version",
  "Run {highlight}awakened auth list{/highlight} to see all configured providers",
  "Run {highlight}awakened agent create{/highlight} for guided agent creation",
  "Use {highlight}/awakened{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}awakened github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/awakened fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.awakened/themes/{/highlight} directory",
  ...
  "Run {highlight}awakened debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}/plugin marketplace add owner/repo{/highlight} to add Claude Code-compatible marketplaces",
  "Use {highlight}/plugin{/highlight} for the plugin hub — install, browse marketplaces, manage plugins",
  "Use {highlight}/yolo{/highlight} or {highlight}--dangerously-skip-permissions{/highlight} to auto-approve permissions (dangerous)",
  "Run {highlight}/doctor{/highlight} for a quick health check (version, MCP, YOLO state)",
  "Browse marketplaces from the plugin manager or run {highlight}awakened plugin list --available{/highlight}",
  "Run {highlight}/awakened{/highlight} to toggle 18 auto-capability packs (browser, TDD, AWS, …)",
  "Built-in skills: {highlight}awakened-subagents{/highlight}, {highlight}superpowers{/highlight}, {highlight}code-review{/highlight} — use {highlight}/skills{/highlight}",
  "Run {highlight}/awakened-subagents{/highlight} to force task-tool dispatch (orchestrator, builder, explore, …)",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/Tlkh201313/AWAKENED-CLI{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with Awakened Zen for curated, tested models",
  "Commit your project's {highlight}AGENTS.md{/highlight} file to Git for team sharing",
  "Use {highlight}/review{/highlight} to review uncommitted changes, branches, or PRs",
  "Use {highlight}/commit{/highlight} to draft a conventional commit message from your changes",
  "Use {highlight}/debug{/highlight} to investigate a failing test, build, or runtime error",
  "Use {highlight}/handoff{/highlight} to summarize progress before starting a fresh session",
  "Use {highlight}/remember{/highlight} or {highlight}/mem{/highlight} for awakened-memory cross-session notes",
  "Use {highlight}/mem-search{/highlight} to search saved memories before re-deciding",
  "Subagents: mention {highlight}@reviewer{/highlight}, {highlight}@debugger{/highlight}, or {highlight}@writer{/highlight} in prompts for focused work",
  (shortcuts) => `Use ${commandText("/usage", shortcuts.sessionUsage())} to see session token and cost breakdown`,
  "Use {highlight}/plan{/highlight} or {highlight}/build{/highlight} to switch between planning and implementation agents",
